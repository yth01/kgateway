...
func getPolicyStatusFn(
	cl kclient.Client[*kgateway.DirectResponse],
...
func patchPolicyStatusFn(
	...
	return func(ctx context.Context, nn types.NamespacedName, policyStatus gwv1.PolicyStatus) error {
		...
		if _, err := cl.UpdateStatus(&kgateway.DirectResponse{
			...
			return fmt.Errorf("error updating status for DirectResponse %s: %w", nn.String(), err)
		}
		return nil
	}
}
