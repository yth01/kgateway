...
When the feature is enabled with `KGW_GLOBAL_POLICY_NAMESPACE=kgateway-system`, the [configuration](/pkg/kgateway/translator/gateway/testutils/inputs/traffic-policy/label_based.yaml) enables the `HTTPRoute` `example-route` in the `infra` namespace to attach to the `TrafficPolicy` `global-policy` in the `kgateway-system` namespace.
